import { setEndCall, setIncomingVideoCall, setVideoCall } from "@/redux/auth/authSlice";
import Image from "next/image";
import React from "react";
import { useDispatch, useSelector } from "react-redux";

function IncomingVideoCall() {
  const { incomingVideoCall, socket, userInfo, videoCall } = useSelector((state) => state.auth);
  ...
  return (
    <div className="h-25 w-80 fixed bottom-8 mb-0 right-6 z-50 rounded-lg flex gap-5 items-center justify-center bg-conversation-panel-background text-white drop-shadow-2xl border-icon-green border-2 py-3">
      <div className="">
        <Image src={incomingVideoCall.profilePicture} height={80} width={80} alt="avatar" />
      </div>
      <div>
